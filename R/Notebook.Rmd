...
widebagmort <- widebagmort %>%
  mutate_at(vars(May, June, July, August, September), ~replace_na(., 0))
...
Double check that all bags are accounted for. There should be 8 bags per colour site per sampling event = 48 as of September 2023
...
mean_survival_proportion %>% ggplot(aes(x=factor(Month_survivor, level=c("April_survivors", 'May_survivors', 'June_survivors', 'July_survivors', 'August_survivors', 'Sept_survivors')), ###add new months here**********
...
ind_surv_dat <- ind_surv_dat %>% select(Site.x, Bag_Col.x, Bag_no.x, OYS_ID,
                                        ...
                                        August_status,
                                        September_status)
ind_surv_dat <- ind_surv_dat %>%
  mutate_all(~ ifelse(is.na(.), 1, .))#replace NA with 1 to indicate that they are dead
...
survival_dat <- ind_surv_dat %>% pivot_longer(cols=c(April_status, May_status, June_status,
                                              July_status, August_status, September_status),
                                              names_to = "Month",
                                              values_to = "event")
...
Now I can finally actually look at survival
