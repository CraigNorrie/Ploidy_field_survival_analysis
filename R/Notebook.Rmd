...
#the next step is to uncount the data from the remaining months
no_survivorsexpanded_allmonths_noapril <- no_survivors %>% 
  filter(Month_survivor != 'April_survivors')

#now I want to repeat this 5 times, once for each individual  
no_survivorsexpanded <- no_survivorsexpanded %>% slice(rep(1:n(), each = 5))
#Now I want 
no_survivorsexpanded
no_survivorsexpanded <- no_survivors %>% 
  uncount(No_survivors)
no_survivorsexpanded$ID <- paste0(no_survivorsexpanded$Site, no_survivorsexpanded$Bag_Col, no_survivorsexpanded$Bag_no, "_",  )  #give everyone an ID number
